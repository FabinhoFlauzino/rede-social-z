...
import { tempoDesdePost } from "@/utils/format-relative"
  ...
  return (
    <div className="flex gap-2 p-6 border-b-2 border-gray-900">
      ...
      <div className="flex-1">
        <div className="flex flex-col lg:flex-row flex-wrap justify-center lg:items-center lg:justify-start gap-x-3">
          <div className="font-bold text-lg">
            <Link href={`/${tweet.user.slug}`}>{tweet.user.name}</Link>
          </div>
          <div className="text-xs text-gray-500">@{tweet.user.slug} - {tempoDesdePost(tweet.dataPost)}</div>
        </div>

        <div className="py-4 text-lg">{tweet.body}</div>
        {tweet.image && (
          <div className="w-full">
            <img src={tweet.image} alt={tweet.body} className="w-full rounded-2xl" />
          </div>
        )}
